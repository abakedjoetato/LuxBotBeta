...
class SettingsView(discord.ui.View):
    ...
    async def prune_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        pruned_settings = []
        # Iterate over a copy of keys since we might modify the cache
        channel_keys_to_check = [key for key in self.bot.settings_cache.keys() if key.endswith('_channel_id')]

        for key in channel_keys_to_check:
            channel_id = self.bot.settings_cache.get(key)
            if channel_id and not self.bot.get_channel(int(channel_id)):
                # Channel is missing, so prune it
...
class AdminCog(commands.Cog):
    ...
    async def show_settings(self, interaction: discord.Interaction):
        ...
        # Get all keys, including ones not in the cache but in the DB, for a complete picture
...
class AdminCog(commands.Cog):
    ...
    async def show_settings(self, interaction: discord.Interaction):
        ...
        channel_keys_to_check = sorted([key for key in self.bot.settings_cache.keys() if key.endswith('_channel_id')])
        ...
        if not description_lines:
            embed.description = "No channel settings found in the cache."
        else:
            embed.description = "\n".join(description_lines)
